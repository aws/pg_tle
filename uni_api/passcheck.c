...
static void passcheck_check_password_hook(const char *username, const char *shadow_pass, PasswordType password_type, Datum validuntil_time, bool validuntil_null)
{
	...
	{
		// Allow skipping if feature is not required to be on across the cluster
	...
	PG_TRY();
	{
		...
		// Assume function accepts the proper argument, it'll error when we call out to SPI_exec
		...
		// Protect against new password types being introduced and out-of-index access
		if (password_type > 2)
			ereport(ERROR,
	...
	PG_CATCH();
	{
		// Hide information on the err other than the err message to prevent passwords
		// from being logged.
		errhidestmt(true);
		errhidecontext(true);
